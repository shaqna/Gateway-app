...
class MainActivity : AppCompatActivity() {
    ...
    private fun processIntent() {
        val bus = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            intent.getParcelableArrayListExtra(BUS, Bus::class.java)
        } else {
        ...
        bus?.let {
            viewModel.setBusTarget(it)
        }
        busStop?.let {
            viewModel.setBusStopTarget(it)
        }
    }
